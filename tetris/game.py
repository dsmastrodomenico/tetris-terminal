...
class Game:
    ...
    def new_piece(self):
        ...
        # Inicia la pieza en el centro superior
    ...
    def update(self):
        ...
        if not self.board.is_valid_move(self.current_piece):
            self.current_piece.y -= 1 # Retrocede
            self.board.place_piece(self.current_piece)
            
            lines_cleared = self.board.clear_lines()
            self.score += lines_cleared * 100 # Puntuación simple

            self.current_piece = self.new_piece()
            if not self.board.is_valid_move(self.current_piece):
                self.game_over = True # No hay espacio para una nueva pieza
    ...
    def handle_input(self, key):
        ...
        if key == 'KEY_LEFT':
            ...
            if not self.board.is_valid_move(self.current_piece):
                self.current_piece.x += 1 # Revertir si es inválido
        elif key == 'KEY_RIGHT':
            self.current_piece.x += 1
            if not self.board.is_valid_move(self.current_piece):
                self.current_piece.x -= 1
        elif key == 'KEY_DOWN':
            self.update() # Acelera la caída
        elif key == 'KEY_UP':
            self.current_piece.rotate()
            if not self.board.is_valid_move(self.current_piece):
                # Intenta revertir la rotación (esto es una simplificación)
                # Una implementación completa requeriría múltiples intentos de rotación
                self.current_piece.rotate()
                self.current_piece.rotate()
                self.current_piece.rotate()
    ...
    def draw(self, stdscr):
        ...
        stdscr.addstr(2, self.board.width * 2 + 5, f"Score: {self.score}")
        if self.game_over:
